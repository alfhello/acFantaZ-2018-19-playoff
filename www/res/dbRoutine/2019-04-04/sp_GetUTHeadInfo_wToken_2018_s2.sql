CREATE DEFINER=`ascdphk1_admin`@`218.253.202.194` PROCEDURE `sp_GetUTHeadInfo_wToken_2018_s2`(in inTeamKey varchar(50), in inGrpKey varchar(50))
BEGIN
...
, trM.TokenAvailable as TokenAvailable
...
left join ( 
select ascFanta_TeamID, ascFanta_TeamName, ascFanta_Group, ascFanta_GroupKey, count(ascFanta_TradeToken_Key) as TokenAvailable from ascFanta_TradeToken_Master_2018_s2
where ascFanta_TeamId = inTeamKey and ascFanta_GroupKey = inGrpKey 
and ascFanta_TradeToken_usedDate is null and ascFanta_TradeToken_usedAction is null and ascFanta_TradeToken_Active = '1'
group by ascFanta_TeamId
) trM on a.ascFanta_TeamId = trM.ascFanta_TeamId and a.ascFanta_TeamName = trM.ascFanta_TeamName and b.ascFanta_Group = trM.ascFanta_Group and b.ascFanta_GroupKey = trM.ascFanta_GroupKey
